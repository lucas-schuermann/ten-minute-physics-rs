...
pub fn fs(frag_coord: Vec2) -> Vec4 {
    let mut uv = (frag_coord - 0.5 * vec2(1600.0, 900.0)) / 900.0;
    uv.y = -uv.y;
...
pub fn main_fs(#[spirv(frag_coord)] in_frag_coord: Vec4, output: &mut Vec4) {
    ...
    *output = fs(frag_coord);
}

#[spirv(fragment)]
pub fn main_fs_2(#[spirv(frag_coord)] in_frag_coord: Vec4, output: &mut Vec4) {
    let frag_coord = vec2(in_frag_coord.x + 1.0, in_frag_coord.y + 1.0);
    *output = fs(frag_coord);
}
